...
function CapabilityIcon({ path }: CapabilityIconProps) {
  return (
    <svg
      className="h-6 w-6"
      fill="none"
...
interface CapabilityCardContentProps {
  card: CapabilityCard
  titleCls: string
  descCls: string
}

function CapabilityCardContent({ card, titleCls, descCls }: CapabilityCardContentProps) {
  return (
    <>
...
interface CapabilityCardProps {
  ...
  editMode: boolean
  reveal: boolean
  onCardChange?: (updated: CapabilityCard) => void
  onOpen: (card: CapabilityCard) => void
}
...
export default function CapabilitiesGrid({
  ...
  const [gridRef, gridInView] = useInView<HTMLDivElement>({ enabled: !reducedMotion, threshold: 0.05 })
  const visibleCards = cards.slice(0, MAX_CAPABILITY_CARDS)
  const revealCards = reducedMotion || gridInView

  const openCard = (card: CapabilityCard) => {
