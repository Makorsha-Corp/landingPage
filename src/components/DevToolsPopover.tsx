...
import type { RefObject, ReactNode, ChangeEvent } from 'react'
...
interface BackdropOpacitySliderProps {
  label: string
  theme: string
  backdropOpacity?: BackdropOpacity
  onChange?: (themeKey: string, value: number) => void
}

function BackdropOpacitySlider({ label, theme, backdropOpacity, onChange }: BackdropOpacitySliderProps) {
  ...
  return (
    <label className="flex min-w-0 items-center gap-3 rounded-xl border border-border/60 bg-muted/30 px-3 py-2.5 text-sm text-muted-foreground">
      ...
      <input
        ...
        step={1}
        value={value}
        onChange={(event: ChangeEvent<HTMLInputElement>) => onChange(themeKey, Number(event.target.value))}
...
interface TransitionSpeedSliderProps {
  label: string
  helperText?: string
  speed?: number
  defaultSpeed: number
...
function TransitionSpeedSlider({ label, helperText, speed, defaultSpeed, onChange, ariaLabel }: TransitionSpeedSliderProps) {
  if (!onChange) return null
...
interface CollapsibleDevSectionProps {
  title: string
  description?: string
  defaultOpen?: boolean
  children?: ReactNode
...
function CollapsibleDevSection({ title, description, defaultOpen = false, children }: CollapsibleDevSectionProps) {
...
interface ViewportLayoutReadoutProps {
  isMobileTour: boolean
}

function ViewportLayoutReadout({ isMobileTour }: ViewportLayoutReadoutProps) {
  const signals = useViewportLayoutSignals()
  const tailwindMdProbeRef = useRef<HTMLSpanElement>(null)
  const [tailwindMdActive, setTailwindMdActive] = useState<boolean | null>(null)
...
interface HeroOverlayScrimSliderProps {
  theme: string
  heroOverlayScrimStrength?: BackdropOpacity
  onChange?: (themeKey: string, value: number) => void
  disabled?: boolean
}

function HeroOverlayScrimSlider({ theme, heroOverlayScrimStrength, onChange, disabled = false }: HeroOverlayScrimSliderProps) {
  if (!onChange) return null
...
interface DevToolsFloatingPanelProps {
  open: boolean
  onClose: () => void
  ...
  factoryPanMode?: boolean
  onToggleFactoryPanMode?: () => void
  mobileCameraPanMode?: boolean
  onToggleMobileCameraPanMode?: () => void
  isMobileTour?: boolean
  tourTransitionSpeed?: number
  onTourTransitionSpeedChange?: (value: number) => void
  tourCardContentSpeed?: number
  onTourCardContentSpeedChange?: (value: number) => void
  heroOverlayScrimStrength?: BackdropOpacity
  onHeroOverlayScrimStrengthChange?: (themeKey: string, value: number) => void
  heroOverlayScrimStyle?: string
  onHeroOverlayScrimStyleChange?: (style: string) => void
  heroOverlayScrimStyles?: LabeledOption[]
  tourBackdropOpacity?: BackdropOpacity
  onTourBackdropOpacityChange?: (themeKey: string, value: number) => void
  sectionsBackdropOpacity?: BackdropOpacity
  onSectionsBackdropOpacityChange?: (themeKey: string, value: number) => void
  sectionBackdrops?: SectionBackdrops
  backdropSections?: LabeledOption[]
  onToggleSectionBackdrop?: (sectionId: string) => void
  onCopyForCode?: () => Promise<{ hadChanges?: boolean } | void>
  rainbowColorPreset?: string
  onRainbowColorPresetChange?: (preset: string) => void
  rainbowColorPresets?: LabeledOption[]
  lightSignUpVariant?: string
  onLightSignUpVariantChange?: (variant: string) => void
  darkSignUpVariant?: string
  onDarkSignUpVariantChange?: (variant: string) => void
  signUpButtonVariants?: LabeledOption[]
  waitlistFabStyle?: string
  onWaitlistFabStyleChange?: (style: string) => void
  waitlistFabStyles?: LabeledOption[]
  brandLogoLightVariant?: string
  onBrandLogoLightVariantChange?: (variant: string) => void
  brandLogoLightVariants?: LabeledOption[]
  perfHudEnabled?: boolean
  onTogglePerfHud?: () => void
  showPerfHudToggle?: boolean
}
...
}: DevToolsFloatingPanelProps) {
  const boundsRef = useRef<HTMLDivElement>(null)
  const panelRef = useRef<HTMLDivElement>(null)
  const [copyState, setCopyState] = useState<'idle' | 'copied' | 'none' | 'failed'>('idle')
...
export interface DevToolsPopoverProps extends Omit<DevToolsFloatingPanelProps, 'open' | 'onClose' | 'buttonRef'> {
  showTrigger?: boolean
  open?: boolean
