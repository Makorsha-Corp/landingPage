...
function SectionDots({ sections, activeIndex, animateHint = false }: SectionDotsProps) {
  return (
    <span
      className={cn(
...
export default function LandingMobileMenu({ sections, activeSection, onNavigate }: LandingMobileMenuProps) {
  ...
  const dismissFromBackdrop = (event: ReactMouseEvent<HTMLDivElement> | ReactPointerEvent<HTMLDivElement>) => {
    event.preventDefault()
