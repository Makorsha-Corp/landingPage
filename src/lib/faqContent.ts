...
export function isPublishableFaq(item: FaqItem): boolean {
  return !String(item.answer).trim().startsWith('TODO')
}

export function mergeFaqFromSaved(saved: Partial<Faq> | null | undefined): Faq {
...
export function faqForStorage(faq: Faq): Faq {
  return {
