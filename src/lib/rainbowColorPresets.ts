...
export interface RainbowColorPreset {
  id: string
  label: string
...
export const RAINBOW_COLOR_PRESET_LIST: RainbowColorPreset[] = [
  {
    id: 'brand',
    label: 'Brand',
    colors: {
...
const presetById = Object.fromEntries(
  RAINBOW_COLOR_PRESET_LIST.map((preset) => [preset.id, preset]),
) as Record<string, RainbowColorPreset>

export function getRainbowColorPreset(id: string): RainbowColorPreset {
  return presetById[id] ?? presetById[DEFAULT_RAINBOW_COLOR_PRESET]
}

export function getWaitlistShineColors(id: string = DEFAULT_RAINBOW_COLOR_PRESET): string[] {
  return getRainbowColorPreset(id).shineColors
}

export function clearRainbowColorPresetOverrides(): void {
  for (let i = 1; i <= 5; i += 1) {
    document.documentElement.style.removeProperty(`--color-${i}`)
  }
}

export function applyRainbowColorPreset(id: string = DEFAULT_RAINBOW_COLOR_PRESET): void {
  if (id === 'magicui') {
