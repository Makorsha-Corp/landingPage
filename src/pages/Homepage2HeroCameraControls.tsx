...
export type CameraState = CameraPosition
...
export const DEFAULT_HERO_CAMERA: CameraPosition = {
  fx: 0.5,
  fy: 0.45,
  scale: 0.8,
}
...
export const DEFAULT_HERO_MOBILE_CAMERA: CameraPosition = {
  fx: 0.5,
  fy: 0.48,
  scale: 1,
}

export const HERO_CAMERA_LIMITS: CameraLimits = {
  fxMin: 0.2,
  fxMax: 0.8,
  fyMin: 0.2,
  fyMax: 0.8,
...
export const TOUR_CAMERA_LIMITS: CameraLimits = {
  fxMin: 0.1,
  fxMax: 0.9,
  fyMin: 0.1,
  fyMax: 0.9,
...
export function normalizeCamera(
  ...
  limits: CameraLimits,
  defaults: Partial<CameraPosition> = {},
): CameraPosition {
  return {
...
export function normalizeHeroCamera(camera: Partial<CameraPosition> | null | undefined): CameraPosition {
  return normalizeCamera(camera, HERO_CAMERA_LIMITS, DEFAULT_HERO_CAMERA)
}

export function normalizeHeroMobileCamera(camera: Partial<CameraPosition> | null | undefined): CameraPosition {
...
interface Homepage2HeroCameraControlsProps {
...
}: Homepage2HeroCameraControlsProps) {
  const safe = normalizeCamera(camera, limits)

  const nudge = (dFx: number, dFy: number) => {
    onChange({
      ...safe,
      ...
      <div>
        ...
        <div className="flex items-center gap-2">
          ...
          <input
            ...
            step={scaleStep}
            value={safe.scale}
            onChange={(e: ChangeEvent<HTMLInputElement>) => onChange({ ...safe, scale: Number(e.target.value) })}
